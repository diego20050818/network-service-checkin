"""Build the teacher/student PDF manual from verified 0.3.0 UI screenshots."""
...
heading('3　查看一周班表')
shot('04-周班表','图 3　周班表按日期、时间和负责人显示班次',345)
steps('点击“看板”。点击周班表的左右箭头切换周次，点击“今天”回到本周。',
      '沿时间轴上下滚动，查看班次时间、负责人和到岗状态。',
      '需要查找某类班次或某位同学时，使用上方“班次”和“成员”筛选。')
table([['颜色','含义'],['绿色 到岗','该席位已签到。'],['红色 未到岗','班次已开始，该席位尚未签到。'],['灰色 未到班','班次尚未开始。']],[0.28,0.72])
p('顶部日期范围用于下方工时汇总；周班表的显示日期由左右箭头单独切换。', 'small')

heading('4　核对出勤与工时')
shot('05-工时汇总','图 4　查看已签到工时、到岗率和成员汇总',340)
...
p('工时按有效签到的班次累加。撤销记录后，该条记录不再计入工时；发现错误时按第 8 页处理。')

heading('5　导入排班和成员信息')
shot('06-导入排班和成员','图 5　在“排班与成员”导入 Excel 文件',340)
...
shot('17-补记表单','图 8　选择班次席位和实际人员后保存补记',220)
...
heading('8　更正 撤销和恢复签到记录')
shot('18-记录明细','图 9　记录明细中的人员更正、撤销与恢复入口',365)
p('选错了实际人员', 'h2')
...
p('需要更正打卡时间', 'h2')
p('当前页面不能直接修改打卡时间。由负责老师核实后，撤销错误记录，再按第 7 页补记正确时间。')
p('改错后，重新进入“看板”和月报页面核对结果。此前已导出的文件需要重新导出。', 'small')

heading('9　填写月报信息并导出')
shot('10-月报信息与文件清单','图 10　本月信息和文件清单',340)
...
heading('10　填写部门工作报表')
shot('11-工作报表编辑','图 11　“完成的工作”可填写多条内容',215)
...
heading('11　核对绩效评分')
shot('12-绩效评分','图 12　逐人修改评分，可填写优秀员工推荐',335)
...
heading('12　核对工资工作量和工时文件')
shot('13-工资工作量','图 13　工资考核表同时显示系统工时和可编辑的工作量',300)
steps('在“文件清单”点击“工资考核表”。',
      '核对“系统工时”和“工作量”。工作量默认使用系统工时，以 h 表示小时，例如 8h。',
      '需要调整时，在“工作量”中填写老师确认的数值，备注中写明原因，等待“已保存”。')
p('修改工作量只影响工资考核表。工时记录表继续采用签到计算结果；已经手动修改的工作量会保留，重新核对签到后也要检查这一列。')
p('其他输出文件', 'h2')
table([
 ['文件','格式','检查内容'],
...
heading('13　设置自启动和文件目录')
shot('14-系统与文件位置','图 14　开机自启动和文件位置设置',345)
p('开机自启动', 'h2')
...
heading('14　查看当前使用的文件')
shot('15-当前使用文件','图 15　查看排班、员工资料和输出模板的文件名及状态',315)
...
heading('15　备份与恢复')
shot('16-备份管理','图 16　备份列表显示时间、类型和是否完整',200)
p('创建备份', 'h2')
