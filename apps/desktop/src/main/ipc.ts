...
const reportDraftSchema: z.ZodType<ReportDraft> = z.object({
  ...
  workItems: z.tuple([z.string().max(5000), z.string().max(5000), z.string().max(5000)]),
  questions: z.tuple([z.string().max(5000), z.string().max(5000), z.string().max(5000)]),
  reflections: z.tuple([z.string().max(5000), z.string().max(5000), z.string().max(5000)]),
  plans: z.tuple([z.string().max(5000), z.string().max(5000), z.string().max(5000)]),
  ...
  selectedFiles: z.array(z.enum(["workReport", "performance", "timeRecord", "schedule", "wageAssessment"])).min(1),
});

function invoke<T extends keyof CheckinApi>(channel: string, handler: (...args: unknown[]) => ReturnType<CheckinApi[T]>): void {
...
export function registerIpcHandlers(context: ServiceContext): void {
  ...
  const isPersistedOutputDirectory = (directory: string): boolean => {
    const rows = context.store.prepare("SELECT payload_json FROM report_drafts").all() as unknown as Array<{ payload_json: string }>;
    return rows.some((row) => {
      try {
        return resolve((JSON.parse(row.payload_json) as ReportDraft).outputDirectory) === resolve(directory);
      } catch {
        return false;
      }
    });
  ...
  const validateOutputDirectory = (directory: string): void => {
    ...
    const isDefaultDirectory = resolve(context.settings.getStorage().defaultOutputDirectory) === normalized;
    if (!chosenOutputDirectories.has(normalized) && !isPersistedOutputDirectory(normalized) && !isDefaultDirectory) {
  ...
  invoke(IPC_CHANNELS.bootstrap, async (now) => {
    const nowValue = z.string().datetime({ offset: true }).optional().parse(now);
  ...
  invoke(IPC_CHANNELS.currentShifts, async (now) => context.attendance.getCurrentAndNext(optionalIsoDateTime.parse(now)));
  invoke(IPC_CHANNELS.shiftsForDate, async (date) => context.attendance.getShiftsForDate(isoDate.parse(date)));
  invoke(IPC_CHANNELS.checkIn, async (shiftId, selections, now) => {
    ...
    return context.attendance.checkIn(parsed.shiftId, parsed.selections, parsed.now);
  });
  ...
  invoke(IPC_CHANNELS.correctRecord, async (recordId, memberId) => context.attendance.correctMember(id.parse(recordId), id.parse(memberId)));
  invoke(IPC_CHANNELS.revokeRecord, async (recordId) => context.attendance.revoke(id.parse(recordId)));
  invoke(IPC_CHANNELS.restoreRecord, async (recordId) => context.attendance.restore(id.parse(recordId)));
  invoke(IPC_CHANNELS.manualRecord, async (input) =>
    context.attendance.addManual(
      z.object({ slotId: id, memberId: id, historicalPunchTime: z.string().datetime({ offset: true }).nullable().optional() }).parse(input),
    ),
  );
  ...
  invoke(IPC_CHANNELS.importSchedule, async (input) => {
    ...
    return context.schedule.import(result.filePaths[0], parsed.month, parsed.effectiveDate, context.settings.get());
  });
  invoke(IPC_CHANNELS.saveImportTemplate, async (templateKind) => {
    const parsedKind = z.enum(["schedule", "members"]).parse(templateKind);
    const fileName = parsedKind === "schedule" ? "网络服务小组排班导入模板.xlsx" : "网络中心员工信息表格.xlsx";
    const sourcePath = join(context.templateDirectory, fileName);
    const result = await dialog.showSaveDialog({
      title: parsedKind === "schedule" ? "保存排班导入模板" : "保存人员信息模板",
      defaultPath: join(app.getPath("downloads"), fileName),
      filters: [{ name: "Excel 工作簿", extensions: ["xlsx"] }],
    });
    if (result.canceled || !result.filePath) return null;
    if (resolve(sourcePath) !== resolve(result.filePath)) await copyFile(sourcePath, result.filePath);
  ...
  invoke(IPC_CHANNELS.chooseStorageDirectory, async (storageKind) => {
    ...
    const result = await dialog.showOpenDialog({
      ...
      defaultPath: parsedKind === "output" ? current.defaultOutputDirectory : current.backupDirectory,
  ...
  invoke(IPC_CHANNELS.reportDraft, async (year, month) => context.reports.getDraft(z.number().int().parse(year), z.number().int().parse(month)));
  ...
  invoke(IPC_CHANNELS.previewReports, async (draft) => context.reports.preview(reportDraftSchema.parse(draft)));
  invoke(IPC_CHANNELS.chooseOutput, async () => {
    const result = await dialog.showOpenDialog({ title: "选择月报输出目录", properties: ["openDirectory", "createDirectory"] });
    if (result.canceled || !result.filePaths[0]) return null;
    const directory = resolve(result.filePaths[0]);
    chosenOutputDirectories.add(directory);
    return directory;
  });
  invoke(IPC_CHANNELS.exportReports, async (draft) => {
    const parsed = reportDraftSchema.parse(draft);
    validateOutputDirectory(parsed.outputDirectory);
    return context.reports.export(parsed);
  });

  invoke(IPC_CHANNELS.openPath, async (path) => {
    const value = z.string().min(1).max(2000).parse(path);
    const exportRows = context.store.prepare("SELECT output_directory FROM export_batches").all() as unknown as Array<{ output_directory: string }>;
    const allowed = context.storage.managedRoots().some((root) => inside(root, value))
      || exportRows.some((row) => inside(row.output_directory, value));
    if (!allowed) throw new Error("拒绝打开未授权路径");
    return shell.openPath(value);
  });
  invoke(IPC_CHANNELS.openData, async () => shell.openPath(context.dataDirectory));
  ...
  invoke(IPC_CHANNELS.restoreManagedBackup, async (path) => {
    await context.backup.restoreManaged(z.string().min(1).max(2000).parse(path));
    relaunchAfterRestore();
    return true;
  });
  invoke(IPC_CHANNELS.restoreBackup, async () => {
    const result = await dialog.showOpenDialog({ title: "选择备份目录", properties: ["openDirectory"] });
