...
describe("ScheduleService", () => {
  it("保存源文件、建立班次席位，并对相同文件保持幂等", async () => {
    ...
    try {
      ...
      const service = new ScheduleService(
        store,
        members,
        join(directory, "sources"),
      );
      const first = await service.import(source, "2026-08", "2026-08-01", {
        attendanceMode: "lenient",
        lateThresholdMinutes: 15,
        latePenaltyPoints: 1,
      });
      const second = await service.import(source, "2026-08", "2026-08-01", {
        attendanceMode: "late_mark",
      ...
      expect(
        (
          store
            .prepare("SELECT COUNT(*) AS count FROM schedule_imports")
            .get() as { count: number }
        ).count,
      ).toBe(1);

      // Simulate an incomplete legacy import. Normal v4 deletes are protected by the source snapshot FK.
      store
        .prepare(
          "DELETE FROM shift_sources WHERE shift_id IN (SELECT id FROM shifts WHERE kind = 'weekend')",
        )
        .run();
      store.prepare("DELETE FROM shifts WHERE kind = 'weekend'").run();
      const repaired = await service.import(source, "2026-08", "2026-08-01", {
        attendanceMode: "lenient",
        lateThresholdMinutes: 15,
        latePenaltyPoints: 1,
      });
      ...
      expect(
        (
          store
            .prepare(
              "SELECT COUNT(*) AS count FROM shifts WHERE kind = 'weekend'",
            )
            .get() as { count: number }
        ).count,
      ).toBeGreaterThan(0);
      expect(
        (
          await service.import(source, "2026-08", "2026-08-01", {
            attendanceMode: "lenient",
            lateThresholdMinutes: 15,
            latePenaltyPoints: 1,
          })
  ...
  it("重新导入完全匹配班次时继承请假与单次增员", async () => {
    ...
    try {
      ...
      await service.import(source, "2099-08", "2099-08-01", {
        attendanceMode: "lenient",
        lateThresholdMinutes: 15,
        latePenaltyPoints: 1,
      });
      const oldShift = store
        .prepare(
          "SELECT id FROM shifts WHERE active = 1 ORDER BY date, start_time LIMIT 1",
        )
        .get() as { id: string };
      const original = attendance.getShift(oldShift.id)!;
      const originalSlot = original.slots[0]!;
      adjustments.createLeave(
        { slotId: originalSlot.id, reason: "课程冲突" },
        new Date("2099-07-01T00:00:00+08:00"),
      );
      ...
      const result = await service.import(source, "2099-08", "2099-08-01", {
        attendanceMode: "lenient",
        lateThresholdMinutes: 15,
        latePenaltyPoints: 1,
      });
      expect(result.duplicate).toBe(false);
      const newShift = store
        .prepare(
          `
      ...
      expect(newShift.id).toBe(oldShift.id);
      expect(
        attendance
          .getShift(newShift.id)!
          .slots.find((slot) => slot.id === originalSlot.id)?.leave?.reason,
      ).toBe("课程冲突");
      const transferred = attendance.getShift(newShift.id)!;
      expect(
        transferred.slots.some(
          (slot) =>
            slot.source === "manual" &&
            slot.role === "staff" &&
            slot.scheduledMemberId === staff.id,
        ),
      ).toBe(true);
      expect(
        transferred.slots.find(
          (slot) => slot.scheduledMemberId === originalSlot.scheduledMemberId,
        )?.leave?.reason,
      ).toBe("课程冲突");
    } finally {
      store.close();
