...
export class AdjustmentService {
  ...
  getActiveLeaveForSlot(slotId: string): LeaveInfo | null {
    const row = this.store
      .prepare(
        `${LEAVE_SELECT} WHERE lr.shift_slot_id = ? AND lr.status = 'active'`,
      )
      .get(slotId) as LeaveRow | undefined;
  ...
  createLeave(input: LeaveInput, now = new Date()): LeaveRecordView {
    ...
    if (!slot.scheduled_member_id)
      throw new Error("只有已安排成员的席位可以请假");
    if (this.hasActiveAttendance(slot.id))
      throw new Error("该席位已有签到，请先撤销签到");
    if (this.getActiveLeaveForSlot(slot.id))
      throw new Error("该席位已经登记请假");
    ...
    `,
      ...
      .run(
        id,
        slot.id,
        slot.scheduled_member_id,
        input.replacementMemberId ?? null,
        input.reason?.trim() ?? "",
        changedAt,
        changedAt,
      );
  ...
  updateLeave(
    ...
    if (this.hasActiveAttendance(leave.slotId))
      throw new Error("代班人已签到，请先撤销签到");
  ...
  cancelLeave(leaveId: string): void {
    ...
    this.store
      .prepare(
        "UPDATE leave_records SET status = 'cancelled', updated_at = ? WHERE id = ?",
      )
      .run(new Date().toISOString(), leaveId);
  }

  addShiftStaff(input: { shiftId: string; memberId: string }): ShiftView {
    const shift = this.store
      .prepare(
        "SELECT id FROM shifts WHERE id = ? AND active = 1 AND work_type = 'regular'",
      )
    ...
    `,
      ...
      .get(input.shiftId, input.memberId, input.memberId) as
      | { id: string }
      | undefined;
    ...
    this.store.transaction(() => {
      const row = this.store
        .prepare(
          "SELECT COALESCE(MAX(position), 0) AS position FROM shift_slots WHERE shift_id = ?",
        )
  ...
  removeShiftStaff(slotId: string): void {
    ...
    if (this.hasActiveAttendance(slotId))
      throw new Error("该办公人员已有签到，请先撤销签到");
    if (this.getActiveLeaveForSlot(slotId))
      throw new Error("该办公人员已有请假，请先撤销请假");
    this.store.transaction(() => {
      this.store
  ...
  createOvertime(input: OvertimeInput, newShiftId?: string): OvertimeEntryView {
    ...
    `,
      ...
      .get(importId, input.date, input.startTime, input.endTime) as
      | { id: string }
      | undefined;
    let createdSlotId = "";
    this.store.transaction(() => {
      if (!shift) {
        shift = { id: newShiftId ?? randomUUID() };
        this.store
          .prepare(
            `
          INSERT INTO shifts(
        ...
        this.store
          .prepare("UPDATE shifts SET active = 1, cancelled = 0 WHERE id = ?")
          .run(shift.id);
      }
      const duplicate = this.store
        .prepare(
  ...
  cancelOvertime(slotId: string): void {
    ...
    if (this.hasActiveAttendance(slotId))
      throw new Error("该加班已有签到，请先撤销签到");
    this.store.transaction(() => {
      this.store
        .prepare("UPDATE shift_slots SET is_vacant = 1 WHERE id = ?")
        .run(slotId);
      ...
      const remaining = this.store
        .prepare(
          "SELECT COUNT(*) AS count FROM shift_slots WHERE shift_id = ? AND is_vacant = 0",
        )
        .get(row.shift_id) as { count: number };
      if (Number(remaining.count) === 0)
        this.store
          .prepare("UPDATE shifts SET active = 0 WHERE id = ?")
          .run(row.shift_id);
    });
  }
  ...
  private getLeave(id: string): LeaveRecordView {
    const row = this.store
      .prepare(`${LEAVE_SELECT} WHERE lr.id = ?`)
      .get(id) as LeaveRow | undefined;
    if (!row) throw new Error("请假记录不存在");
    return mapLeave(row);
  }

  restoreLeave(id: string): void {
    const leave = this.getLeave(id);
    if (leave.status === "active") return;
    const slot = this.leaveTarget(leave.slotId);
    if (slot.scheduled_member_id !== leave.memberId)
      throw new Error("原席位人员已变化，不能恢复旧请假");
    if (
      this.hasActiveAttendance(leave.slotId) ||
      this.getActiveLeaveForSlot(leave.slotId)
    )
      throw new Error("该席位已有签到或新的请假");
    // An ended shift can restore its historical replacement, but cannot assign a new one.
    this.validateReplacement(
      slot,
      leave.replacementMemberId,
      new Date(slot.date + "T00:00:00"),
    );
    this.store
      .prepare(
        "UPDATE leave_records SET status='active',updated_at=? WHERE id=?",
      )
      .run(new Date().toISOString(), id);
  }

  restoreOvertime(slotId: string): void {
    const row = this.store
      .prepare(
        "SELECT ss.shift_id,ss.scheduled_member_id,ss.is_vacant FROM shift_slots ss JOIN shifts s ON s.id=ss.shift_id WHERE ss.id=? AND s.work_type='overtime'",
      )
      .get(slotId);
    if (!row) throw new Error("加班席位不存在");
    if (!this.members.get(String(row.scheduled_member_id))?.active)
      throw new Error("原成员已停用，不能恢复");
    if (!row.is_vacant) return;
    if (
      this.store
        .prepare(
          "SELECT id FROM shift_slots WHERE shift_id=? AND scheduled_member_id=? AND is_vacant=0",
        )
        .get(row.shift_id!, row.scheduled_member_id!)
    )
      throw new Error("该成员已有同班加班安排");
    this.store.transaction(() => {
      this.store
      ...
      this.store
        .prepare("UPDATE shifts SET active=1,cancelled=0 WHERE id=?")
        .run(row.shift_id!);
      this.syncRequiredSlots(String(row.shift_id));
    });
  }
  ...
  private validateReplacement(
    ...
    if (!memberId) return;
    if (`${slot.date}T${slot.end_time}` <= formatLocalDateTimeKey(now))
      throw new Error("已结束班次不能再分配代班人");
    if (memberId === slot.scheduled_member_id)
      throw new Error("代班人不能是原排班成员");
    if (!this.members.get(memberId)?.active)
      throw new Error("代班成员不存在或已停用");
    ...
    `,
      ...
      .get(slot.shift_id, slot.id, memberId, memberId) as
      | { id: string }
      | undefined;
  ...
  private ensureSystemImport(month: string): string {
    const existing = this.store
      .prepare(
        "SELECT id FROM schedule_imports WHERE source_type = 'system' AND source_sha256 = 'system-overtime-' || ?",
      )
