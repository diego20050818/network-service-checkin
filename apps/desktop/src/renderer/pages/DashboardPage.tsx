...
import type { DashboardSnapshot, Member } from "../../shared/contracts";
...
export function DashboardPage({ members }: { members: Member[] }) {
  ...
  const [kind, setKind] = useState<"all" | "desk" | "maintenance" | "weekend">("all");
    ...
    <div className="page-stack">
      ...
      <section className="dashboard-filters">
        ...
        <label>班次<select value={kind} onChange={(event) => setKind(event.target.value as typeof kind)}>
          <option value="all">全部班次</option><option value="desk">工作日坐班</option><option value="weekend">周末坐班</option><option value="maintenance">维修班</option>
        </select></label>
        ...
        <>
          ...
          <section className="dashboard-section">
            ...
            {snapshot.members.length === 0 ? <EmptyState title="暂无成员" description="请先导入成员资料或排班。" /> : (
              <div className="table-scroll"><table><thead><tr><th>姓名</th><th>已签到班次</th><th>坐班工时</th><th>维修工时</th><th>总工时</th><th>迟到</th><th>临时替班</th></tr></thead>
                <tbody>{snapshot.members.map((member) => <tr key={member.memberId}><td><strong>{member.memberName}</strong></td><td>{member.shiftCount}</td><td><Hours minutes={member.deskMinutes + member.weekendMinutes} /></td><td><Hours minutes={member.maintenanceMinutes} /></td><td><strong><Hours minutes={member.totalMinutes} /></strong></td><td>{member.lateCount}</td><td>{member.substituteCount}</td></tr>)}</tbody>
              </table></div>
            )}
          </section>
          ...
          <section className="dashboard-section">
            ...
            {snapshot.records.length === 0 ? <EmptyState title="所选范围暂无签到" description="签到成功后会立即出现在这里。" /> : (
              <div className="table-scroll"><table><thead><tr><th>日期</th><th>班次</th><th>原排班</th><th>实际人员</th><th>打卡时间</th><th>计薪工时</th><th>状态</th></tr></thead>
                <tbody>{snapshot.records.slice(0, 200).map((record) => <tr key={record.id}><td>{record.date}</td><td>{record.label}<small>{record.startTime}-{record.endTime}</small></td><td>{record.scheduledMemberName ?? "空位"}</td><td><strong>{record.actualMemberName}</strong></td><td>{record.punchTime ? new Date(record.punchTime).toLocaleString("zh-CN", { hour12: false }) : "未填写"}</td><td><Hours minutes={record.paidMinutes} /></td><td><StatusPill tone={record.lateStatus === "late" ? "amber" : record.lateStatus === "manual_unjudged" ? "gray" : "green"}>{record.lateStatus === "late" ? "迟到" : record.lateStatus === "manual_unjudged" ? "补记未判定" : "正常"}</StatusPill></td></tr>)}</tbody>
              </table></div>
            )}
          </section>
        </>
